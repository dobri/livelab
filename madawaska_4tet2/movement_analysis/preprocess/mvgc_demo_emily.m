...
etests    = true;  % do experimental (unit-root stationarity) tests?
...
%% SKIP - Generate VAR test data (<mvgc_schema.html#3 |A3|>)
%
...
AT = M;
nvars = size(M,3); % number of variables
...
%ptoc; %data is in the form # of variables (nvars) x # of observations (nob) x # of trials (ntrials)
...
ptic('\n*** tsdata_to_infocrit\n');
[AIC,BIC,moAIC,moBIC] = tsdata_to_infocrit(M,momax,icregmode);
...
ptic('\n*** tsdata_to_var... ');
[A,SIG] = tsdata_to_var(M,morder,regmode);
ptoc;
