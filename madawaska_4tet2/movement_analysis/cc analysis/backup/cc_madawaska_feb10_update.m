...
if strcmp(method_flag,'wcc')
    ...
    sr100 = 100;
    win_len = [5, 5]; %in seconds
    max_lag = [2, 2]; % value of beat (quarter note in piece 1, dotted quarter in piece 2)
end
...
%    'Xpcs_processed','A_processed','X_clean_ml_processed'}; 
%dataTrajs={'X_clean_processed','X_clean_ml_processed'}; 
...
for piecei = 1:numel(D)
    for traji = 1:numel(dataTrajs)
        ...
        for triali=1:num_trials
            ...
            %read in salient point labels from audacity
            switch 1 
                case 0
                    ...
                    for row=1:4
                        for col=1:4
                            if col>row
                                ...
                                %end
                                [wcc,l,t]=corrgram(x,y,maxlag(piecei),window(piecei),overlap(piecei));
                                ...
                                if length(indexR) > 1 %added this in in case max value occurs at multiple lags (e.g., piecei = 1, triali =6, traji = 5)
                                    indexR=round(mean(indexR));
                                end
...
for p=1:2
    subplot(2,2,1+(p-1)*2)
    boxplot(D{p}.A_processed_wcc,reshape(meshgrid(1:8,1:6),1,[])')
end
...
if save_flag==1
    ...
    %dataTrajs={'X_clean_processed_wcc','X_clean_ml_processed_wcc'};
    dataTrajs={'X_processed_wcc'};
    ...
    for traji = 1:numel(dataTrajs)     
        ...
        filename=[dataTrajs{traji},'_feb15_2.xlsx'];
